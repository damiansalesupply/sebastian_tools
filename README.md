...
You do not need to understand the API to use it: configure shops and emails in YAML, set credentials in `.env`, run the script from this folder with **`uv run`**, and collect outputs under `data/`.
...
- **[uv](https://docs.astral.sh/uv/)** and Python **3.12+** (see `pyproject.toml`). Install dependencies with `uv sync` (or let `uv run` sync on demand).
- Run Python in this project with **`uv run python …`** so the lockfile / `pyproject.toml` environment is used (not a random global Python).
...
| `--only-shops SHOP_ID …` | Export only these shop IDs. IDs must already have recipients in `shops.yml`; others are skipped with a warning. |
...
Use `uv run python export_comments.py --help` for the built-in help text.
...
```bash
uv run python export_comments.py
```

**Export only, no email:**

```bash
uv run python export_comments.py --no-sending
```

**Only certain shops (must have recipients in `shops.yml`):**

```bash
uv run python export_comments.py --only-shops 1521 2040 --no-sending
```

**Previous calendar month:**

```bash
uv run python export_comments.py --period last_month --no-sending
```

**Last 14 days (inclusive, ending today):**

```bash
uv run python export_comments.py --period last_n_days --n-days 14
```

**Fixed date range:**

```bash
uv run python export_comments.py --period time-range --from-date 2026-04-01 --to-date 2026-04-15 --no-sending
```

**Full export but send mail only to one address (smoke test):**

```bash
uv run python export_comments.py --only-send-to colleague@example.com
```

**Wider ticket search window (factor 5):**

```bash
uv run python export_comments.py --ticket-window-factor 5
```
